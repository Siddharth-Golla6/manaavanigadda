...
import { ThumbsUp, MapPin, User, Calendar, MessageCircle, X, ChevronLeft, ChevronRight, Expand } from "lucide-react";
...
export default function ProblemDetails() {
  const { id } = useParams();
  const { problems, loading, upvote, addComment } = useProblems();
  const { user } = useAuth();
  ...
  const handleUpvote = async () => {
    if (voted) return;
    try {
      await upvote(problem.id);
      setVoted(true);
    } catch (err) {
      if (err.message.includes("already supported")) setVoted(true);
  ...
  const handleComment = async (e) => {
    ...
    try {
      await addComment(problem.id, comment.trim());
      setComment("");
    } catch (err) {
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      ...
      <div className="glass-card overflow-hidden">
          ...
          <div>
            ...
            >
              <img
                ...
                alt={`${problem.title} — photo ${activePhoto + 1}`}
        ...
        <div className="p-6">
          ...
          <div className="mt-3 flex flex-wrap gap-4 text-sm text-neutral-500 dark:text-neutral-400">
            ...
            <span className="flex items-center gap-1.5"><User size={14} /> Reported by {problem.reportedByName}</span>
            <span className="flex items-center gap-1.5"><Calendar size={14} /> {new Date(problem.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span>
          </div>
          ...
          <div className="mt-5 flex items-center gap-3">
            ...
            >
              <ThumbsUp size={16} /> {voted ? "Supported" : "Support this issue"} · {problem.supportCount}
            </button>
            {problem.assignedVolunteer && (
              <span className="text-sm text-neutral-500">
                Assigned to <strong className="text-neutral-700 dark:text-neutral-200">{problem.assignedVolunteer}</strong>
              </span>
            )}
          </div>
        </div>
      ...
      {problem.progressPhotos?.length > 0 && (
        <GlassCard className="mt-6">
          <h2 className="mb-3 text-sm font-bold text-neutral-800 dark:text-neutral-200">Progress Photos</h2>
          <div className="flex gap-3 overflow-x-auto">
            {problem.progressPhotos.map((p, i) => (
              <img key={i} src={p} alt="Progress" className="h-32 w-48 shrink-0 rounded-lg object-cover" />
            ))}
          </div>
        </GlassCard>
      )}
      ...
      <GlassCard className="mt-6">
        <h2 className="mb-4 text-sm font-bold text-neutral-800 dark:text-neutral-200">Status Timeline</h2>
        <ol className="space-y-4 border-l-2 border-brand-yellow/50 pl-5">
          {problem.timeline.map((t, i) => (
            <li key={i} className="relative">
              ...
              <p className="text-sm font-semibold text-neutral-800 dark:text-neutral-200">{t.status}</p>
      ...
      {(problem.lat || problem.lng) && (
        <GlassCard className="mt-6">
          <h2 className="mb-3 text-sm font-bold text-neutral-800 dark:text-neutral-200">Location</h2>
      ...
      <GlassCard className="mt-6">
        <h2 className="mb-4 flex items-center gap-2 text-sm font-bold text-neutral-800 dark:text-neutral-200">
          <MessageCircle size={16} /> Comments ({problem.comments.length})
        </h2>
        ...
        {user ? (
          ...
          <p className="text-sm text-neutral-500">
            <Link to="/login" className="font-semibold text-brand-red hover:underline">Log in</Link> to add a comment.
          </p>
        )}
      </GlassCard>
      ...
      {lightboxOpen && photos.length > 0 && (
        ...
        >
          ...
          <img
            src={photos[activePhoto]}
            alt={`${problem.title} — full size ${activePhoto + 1}`}
